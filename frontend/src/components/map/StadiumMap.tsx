...
export default React.memo(function StadiumMap({ onLocationClick }: { onLocationClick?: (loc: string) => void }) {
  ...
  const coordinates: Record<string, [number, number]> = {
    ...
    "Gate B": [25.4200, 51.4903],
    "Gate C": [25.4208, 51.4890],
  ...
  if (!isClient) return <div className="w-full h-full bg-zinc-900 rounded-xl animate-pulse flex items-center justify-center">Loading Map...</div>;

  return (
    <div className="w-full h-full relative rounded-xl overflow-hidden glass border-white/10 z-0">
      <MapContainer center={stadiumCenter} zoom={17} style={{ height: "100%", width: "100%" }} zoomControl={false}>
        <TileLayer
        ...
        {Object.entries(coordinates).map(([name, coords]) => {
          ...
          return (
            <Marker key={name} position={coords} eventHandlers={{ click: () => onLocationClick && onLocationClick(name) }} alt={`Map marker for ${name}`} title={name}>
              <Popup className="glass-popup">
                <div className="p-2 min-w-[200px]">
                  <h3 className="font-bold text-lg border-b border-gray-200 pb-1 mb-2">{name}</h3>
                  {crowd ? (
                    <div className="space-y-1 text-sm">
                      <p className="flex justify-between"><span>People:</span> <span className="font-semibold">{crowd.peopleCount}</span></p>
                      <p className="flex justify-between"><span>Density:</span> <span className={`font-semibold text-${color}-500`}>{crowd.density}</span></p>
                      <p className="flex justify-between"><span>Queue:</span> <span className="font-semibold">{crowd.queueTime} mins</span></p>
                    </div>
                  ) : (
