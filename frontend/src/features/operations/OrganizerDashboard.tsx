...
import { useCrowd, useIncidents, useVolunteers, useTransport, useVenue } from "@/hooks/useFirestore";
    ...
    <div className="flex-1 overflow-y-auto p-8 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {[
          ...
          { label: "Active Incidents", value: activeIncidents.length, icon: AlertTriangle, color: "red" },
          { label: "Medical Requests", value: activeIncidents.filter(i => i.type === "Medical").length, icon: BriefcaseMedical, color: "orange" },
          { label: "Available Vols", value: availableVolunteers, icon: ShieldAlert, color: "purple" },
          { label: "Parking Filling", value: transportData.find(t => t.type === "Parking")?.occupancy + "%" || "N/A", icon: Car, color: "zinc" }
        ].map((stat, i) => (
          <div key={i} className={`glass-card p-4 rounded-xl border border-${stat.color}-500/20 bg-white/5`}>
            <div className={`text-${stat.color}-400 mb-2`}><stat.icon size={20} /></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <PredictiveAISection predictions={aiAnalysis?.predicted_problems || []} loading={analyzing} />
        </div>
