      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-6 relative">
        ...
        {sorted.map((inc, i) => (
          <div key={inc.id || i} className="relative z-10 pl-8">
            {/* Timeline dot */}
            <div
              className={`absolute left-0 top-1.5 w-5 h-5 rounded-full border-4 border-black ${getStatusColor(inc.status)}`}
            ></div>
            ...
            <div className="bg-white/5 border border-white/5 p-3 rounded-lg hover:bg-white/10 transition-colors">
              ...
              <div className="flex items-center gap-2 mt-2">
                <span
                  className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${inc.priority === 'Critical' ? 'bg-red-500/20 text-red-400' : inc.priority === 'High' ? 'bg-orange-500/20 text-orange-400' : 'bg-blue-500/20 text-blue-400'}`}
                >
