      ...
      <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-2">
        ...
        <div className="grid grid-cols-2 gap-3 mb-2">
          {transportData.map((t, i) => (
            <div key={i} className="bg-black/30 border border-white/5 p-3 rounded-lg flex items-center justify-between">
              <div className="flex items-center gap-2 text-sm font-medium">
                <span className="text-zinc-400">{getIcon(t.type)}</span>
                {t.type}
              </div>
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${t.status === 'On Time' ? 'bg-green-500/20 text-green-400' : t.status === 'Delayed' ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
        ...
        <div>
          ...
          <div className="flex items-center bg-black/40 border border-white/10 rounded-lg px-3 py-2">
            ...
            <select value={preference} onChange={e => setPreference(e.target.value)} className="bg-transparent border-none outline-none w-full text-sm text-white [&>option]:bg-zinc-900">
        ...
        >
          {loading ? "Calculating..." : <><Navigation size={16} /> Get Travel Advice</>}
        </button>
