  ...
  return (
    <div className="flex flex-col h-full bg-white/5 border border-white/10 rounded-xl p-6 relative overflow-hidden">
      ...
      <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-2">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <div className="flex items-center bg-black/40 border border-white/10 rounded-lg px-3 py-2">
              ...
              <input value={currentLocation} onChange={e => setCurrentLocation(e.target.value)} className="bg-transparent border-none outline-none w-full text-sm text-white" />
            </div>
          </div>
          <div>
            ...
            <div className="flex items-center bg-black/40 border border-white/10 rounded-lg px-3 py-2">
              ...
              <input value={destination} onChange={e => setDestination(e.target.value)} className="bg-transparent border-none outline-none w-full text-sm text-white" />
            </div>
          </div>
        </div>

        <div>
          <label className="block text-xs font-medium text-zinc-400 mb-2 mt-2">Accessibility Profile</label>
          <div className="grid grid-cols-2 gap-2">
            <label className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors border ${needsWheelchair ? 'bg-purple-500/20 border-purple-500/50 text-purple-400' : 'bg-white/5 border-white/10 text-zinc-400'}`}>
              <input type="checkbox" className="hidden" checked={needsWheelchair} onChange={() => setNeedsWheelchair(!needsWheelchair)} />
              <Accessibility size={16} /> <span className="text-xs font-medium">Wheelchair</span>
            </label>
            <label className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors border ${needsVisual ? 'bg-purple-500/20 border-purple-500/50 text-purple-400' : 'bg-white/5 border-white/10 text-zinc-400'}`}>
              <input type="checkbox" className="hidden" checked={needsVisual} onChange={() => setNeedsVisual(!needsVisual)} />
              <Eye size={16} /> <span className="text-xs font-medium">Visual</span>
            </label>
            <label className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors border ${needsHearing ? 'bg-purple-500/20 border-purple-500/50 text-purple-400' : 'bg-white/5 border-white/10 text-zinc-400'}`}>
              <input type="checkbox" className="hidden" checked={needsHearing} onChange={() => setNeedsHearing(!needsHearing)} />
              <Ear size={16} /> <span className="text-xs font-medium">Hearing</span>
            </label>
            <label className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors border ${needsStroller ? 'bg-purple-500/20 border-purple-500/50 text-purple-400' : 'bg-white/5 border-white/10 text-zinc-400'}`}>
              <input type="checkbox" className="hidden" checked={needsStroller} onChange={() => setNeedsStroller(!needsStroller)} />
        ...
        >
          {loading ? "Analyzing Routes..." : <><Navigation2 size={16} /> Generate Accessible Route</>}
        </button>

        {response && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-6 space-y-4">
            <div className="p-4 bg-purple-500/10 border border-purple-500/20 rounded-lg">
              <h4 className="font-semibold text-purple-400 text-xs mb-2">Safe Route ({response.estimated_time})</h4>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white/5 border border-white/10 p-3 rounded-lg">
                ...
                <ul className="text-sm text-green-400 list-disc pl-4">
                  {response.accessible_facilities?.map((f: string, i: number) => <li key={i}>{f}</li>)}
                </ul>
              </div>
              <div className="bg-white/5 border border-white/10 p-3 rounded-lg">
                <h4 className="text-xs font-medium text-zinc-400 mb-1">Rest Areas</h4>
                <ul className="text-sm text-blue-400 list-disc pl-4">
                  {response.rest_areas?.map((r: string, i: number) => <li key={i}>{r}</li>)}
                </ul>
              </div>
            </div>
            ...
            {response.warnings && response.warnings.length > 0 && (
              <div className="bg-orange-500/10 border border-orange-500/20 p-3 rounded-lg">
                ...
                <ul className="text-sm text-orange-300 list-disc pl-4">
                  {response.warnings.map((w: string, i: number) => <li key={i}>{w}</li>)}
                </ul>
              </div>
            )}
