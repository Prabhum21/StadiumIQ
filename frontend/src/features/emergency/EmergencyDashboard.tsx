    ...
    <div className="flex-1 overflow-y-auto p-8 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div
            key={i}
            className={`glass-card p-4 rounded-xl border border-${stat.color}-500/20 bg-white/5 flex items-center justify-between`}
          >
            <div>
              <div className="text-2xl font-bold">{stat.value}</div>
              <div className="text-xs text-zinc-400 mt-1">{stat.label}</div>
            </div>
            <div className={`text-${stat.color}-400 bg-${stat.color}-500/10 p-3 rounded-full`}>
              <stat.icon size={24} />
            </div>
          </div>
        ))}
          ...
          <div className="glass-card p-6 h-[400px] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3">
              {openIncidents.map((inc) => (
                ...
                >
                  <div className="flex justify-between mb-1">
                    ...
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${inc.priority === 'Critical' ? 'bg-red-500/20 text-red-400' : 'bg-orange-500/20 text-orange-400'}`}
                    >
                      {inc.priority}
                    </span>
                  </div>
              ...
              {openIncidents.length === 0 && (
                <p className="text-sm text-zinc-500 text-center py-4">No active incidents.</p>
              )}
            </div>
          </div>
        </div>
